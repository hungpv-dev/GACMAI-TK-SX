...
class LoginController extends Controller
{
    ...
    public function login(Request $request){
        ...
        if($request->ajax()){
            return $request->response()->json($validate->errors(),422);
        }
        if($validate->fails()){
            $validate->flush();
            return back();
    ...
    protected function handleLogin($user, $request, $avatar)
    {
        ...
        if ($user) {
            ...
            if ($user->status == $this->true_status) {
    ...
    public function logout()
    {
        try{
            ...
            // Xóa cookie token
            setcookie('secure_access_token', '', time() - 3600, '/'); // Thay đổi thời gian hết hạn
            setcookie('secure_refresh_token', '', time() - 3600, '/'); // Thay đổi thời gian hết hạn
    ...
    private function generateNewAccessToken($refreshPayload, $token)
    {
        try {
            ...
            if ($user && $userLog && $user->status == $this->true_status) {
                $refreshPayload['exp'] = time() + $this->access_express;
                $refreshPayload['email'] = $user->email;
                $refreshPayload['username'] = $user->name;
