<div class="row">
    <div class='row'>
        <div class="col-md-6 mt-5 col-sm-12 m-auto">
            <h4 class='text-center'>Doanh thu tất toán trong kỳ</h4>
            <div class="chart-all-group" style="min-height:320px"></div>
        </div>
        <div class="col-md-6 mt-5 col-sm-12 m-auto">
            <h4 class='text-center'>Doanh thu thực thu trong kỳ</h4>
            <div class="chart-all-group-ttd" style="min-height:320px"></div>
        </div>
    </div>
    <div class="row mt-5">
        <h4 class='text-center'>Doanh thu tất toán, giá vốn, lợi nhuận</h4>
        <div class="doanh-thu-tt-gv-ln" style="min-height:450px"></div>
    </div>
    <div class='row mt-5'>
        <div class="col-md-4 mt-5 col-sm-12 m-auto">
            <h4 class='text-start'>Doanh thu thực thu trong kỳ</h4>
                    ...
                    <table class="table table-hover table-sm fs-9 mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle text-end text-uppercase">Doanh thu thực thu</th>
                            </tr>
                        </thead>
                        ...
                        <tbody class="list-data" id="data_table_body">
                            @php
                                $total = 0;
                            @endphp
                            @foreach ($chartAllGroupTTD as $item)
                                @php
                                    $total += $item->total_thuc_thu;
                                @endphp
                                <tr>
                                    <td data-label="Nhóm" class='align-middle text-start'>
                                        <a href="/logs/tienthuduoc?group_id={{ $item->group_id }}&dates={{ $dates }}&status=2">
                                            {{ $item->group_name }}
                                        </a>
                                    </td>
                                    <td data-label="Doanh thu" class='align-middle text-end'>
                                        <a href="/logs/tienthuduoc?group_id={{ $item->group_id }}&dates={{ $dates }}&status=2">
                                            <span class='fw-bold text-info'>{{ number_format($item->total_thuc_thu) }}
                                                ₫</span>
                                        </a>
                                    </td>
                                </tr>
        ...
        <div class="col-md-7 mt-5 col-sm-12 m-auto">
            <h4 class='text-start'>Doanh thu tất toán trong kỳ</h4>
            <div class="mx-n4 mx-lg-n6 px-4 px-lg-6 mb-9 bg-body-emphasis border-y mt-2 position-relative top-1">
                <div class="table-responsive quote-table-container scrollbar ms-n1 ps-1">
                    <table class="table table-hover table-sm fs-9 mb-0">
                        <thead>
                            <tr>
                                <th class="align-middle text-start text-uppercase">Nhóm</th>
                                <th class="align-middle text-end text-uppercase">Doanh thu tất toán</th>
                                <th class="align-middle text-end text-uppercase">Giá vốn</th>
                        ...
                        <tbody class="list-data" id="data_table_body">
                            @php
                                ...
                                $total_ln = 0;
                            @endphp
                            @foreach ($chartAllGroup as $item)
                                @php
                                    $total += $item->total_thuc_thu;
                                    $total_cp += $item->total_chi_phi;
                                    $loinhuan = $item->total_thuc_thu - $item->total_chi_phi;
                                    $total_ln += $loinhuan;
                                @endphp
                                <tr>
                                    <td data-label="Nhóm" class='align-middle text-start'>
                                        <a href='/orders?dates={{ $dates }}&status={{ status('order_success') }}&group_id=<?= $item->group_id ?>'>
                                        {{ $item->group_name }}
                                        </a>
                                    </td>
                                    <td data-label="Doanh thu" class='align-middle text-end'>
                                        <a href='/orders?dates={{ $dates }}&status={{ status('order_success') }}&group_id=<?= $item->group_id ?>'>
                                            <span class='fw-bold text-info'>{{ number_format($item->total_thuc_thu) }} ₫</span>
                                        </a>
                                    </td>
                                    <td data-label="Doanh thu" class='align-middle text-end'>
                                        <a href='/orders?dates={{ $dates }}&status={{ status('order_success') }}&group_id=<?= $item->group_id ?>'>
                                            <span class='fw-bold text-warning'>{{ number_format($item->total_chi_phi) }} ₫</span>
                                        </a>
                                    </td>
                                    <td data-label="Doanh thu" class='align-middle text-end'>
                                        <a href='/orders?dates={{ $dates }}&status={{ status('order_success') }}&group_id=<?= $item->group_id ?>'>
                                            <span class='fw-bold text-{{ $loinhuan >= 0 ? 'success' : 'danger' }}'>{{ number_format($loinhuan) }} ₫</span>
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
                            <tr class='fw-bold none-data'>
                                ...
                                <td class='align-middle text-end'>
                                    <span class='fw-bold text-warning'>{{ number_format($total_cp) }} ₫</span>
                                </td>
                                <td class='align-middle text-end'>
                                    <span
                                        class='fw-bold text-{{ $total_ln >= 0 ? 'success' : 'danger' }}'>{{ number_format($total_ln) }}
                                        ₫</span>
                                </td>
                            </tr>
                        </tbody>
                ...
                baseOption: {
                    ...
                    series: [{
                        ...
                        label: {
                            ...
                            fontSize: '12px',
                            fontWeight: 'bold',
                            formatter: params => formatCurrency(params.value) // Format value
                        },
                        barMaxWidth: '80px'
                    }, {
                        name: 'Giá vốn',
