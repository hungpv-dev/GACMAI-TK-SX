...
<div class="content">
    <nav class="mb-2" aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item">Trang thái khách hàng</li>
        </ol>
    </nav>
    ...
    <div>
        ...
        <div class="col-8 m-auto">
            ...
            <p><i>- Ô chọn thứ nhất là trạng thái <b class='text-info'>xác nhận</b>, chuyển sang trạng thái này sẽ tự
                    tạo đơn</i></p>
            <p><i>- Ô chọn thứ hai là trạng thái <b class='text-info'>hoàn thành</b>, chuyển sang trạng thái này sẽ
                    không thể chỉnh sửa nữa</i></p>
                ...
                <div class="col-8 m-auto" data-sortable="data-sortable">
                    ...
                    <div class="sortable-item-wrapper mb-3" data-id="{{ $item->id }}">
                        <div class="input-group">
                            <div title="Khi chuyển sang trạng thái này sẽ tạo đơn hàng" class="input-group-text">
                                <input class="form-check-input mt-0" {{ $item->type == 1 ? 'checked' : '' }}
                                name="success" style="cursor: pointer;" type="radio">
                            </div>
                            <div title="Khi chuyển sang trạng thái này sẽ không thể chỉnh sửa" class="input-group-text">
                                <input class="form-check-input mt-0" {{ $item->type == 2 ? 'checked' : '' }}
                                name="order" style="cursor: pointer;" type="radio">
                            </div>
...
<script>
    document.getElementById('add_new').addEventListener('click', function() {
        ...
        newItem.innerHTML = `
            <div class="input-group">
                <div class="input-group-text" title="Khi chuyển sang trạng thái này sẽ tạo đơn hàng">
                    <input class="form-check-input mt-0" name="success" style="cursor: pointer;" type="radio">
                </div>
        ...
        newItem.querySelectorAll('input')[2].focus();
    });
    ...
    document.getElementById('update').addEventListener('click', function() {
        ...
        document.querySelectorAll('.sortable-item-wrapper').forEach((item, index) => {
            ...
            let type = item.querySelector('input[name="success"]').checked ? 1 : null;
            if(!type){
                type = item.querySelector('input[name="order"]').checked ? 2 : null;
            }
            statusData.push({ id, name, type,bg,color, sort: index + 1 });
        });
        axios.post('/api/customers-status', statusData, {
            headers: {
